...
st.header('Netflix Film Öneri Sistemi')
...
if st.button('Öneriyi Göster'):
    ...
    st.subheader("10 Film Önerisi")
    for index, row in recommended_movie_names.iterrows():
        movie_info = f"{row['title']} ({row['release_year']})"
        st.write(movie_info)
